...
class TicTocToe implements GameInterface{
    ...
    public static void main(String args[]){
		TicTocToe gamePlan = new TicTocToe();
		while(gamePlan.isOn())
	    	gamePlan.play();
	}
    ...
    public boolean lookForWinner(int x,int y){
		...
		if(count(x,y,1,0,t) + count(x,y,-1,0,t)+1>= SIZE)
			return true;
		if(count(x,y,0,1,t) + count(x,y,0,-1,t)+1>= SIZE)
			return true;
		if(count(x,y,1,1,t) + count(x,y,-1,-1,t)+1>= SIZE)
			return true;
		if(count(x,y,-1,1,t) + count(x,y,1,-1,t)+1>= SIZE)
			return true;
		return false;
